...
{
	CDeferredPipeline::CDeferredPipeline(const std::string& vVertShaderPath, const std::string& vFragShaderPath, int vWidth, int vHeight)
		: m_pQuadVAO(CPrimitive::createQuad())
	{
		m_pGeomRenderPass = std::make_shared<CGeometryRenderPass>(vVertShaderPath, vFragShaderPath);
		m_pGeomRenderPass->init(vWidth, vHeight);
		m_pLightingRenderPass = std::make_shared<CLightingRenderPass>(m_pGeomRenderPass->getFrameBuffer(), m_pQuadVAO);
